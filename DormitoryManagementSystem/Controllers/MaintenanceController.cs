...
namespace DormitoryManagementSystem.Controllers
{
    ...
    public class MaintenanceController : Controller
    {
        ...
        public MaintenanceController(AppDbContext context, AuditService audit)
        {
            _context = context;
            _audit = audit;
        }
        ...
        public IActionResult Index()
        {
            ...
            if (User.IsInRole("Student"))
            {
                ...
                query = query.Where(m => m.Student != null && m.Student.FullName == fullName);
            }
        ...
        public IActionResult Create(MaintenanceRequest request)
        {
            ...
            {
                var fullName = User.FindFirst("FullName")?.Value;
                var student = _context.Students.FirstOrDefault(s => s.FullName == fullName);
                if (student == null)
                {
                    TempData["Error"] = "Student profile could not be matched for the logged-in user.";
                    return RedirectToAction(nameof(Index));
            ...
            {
                var fullName = User.FindFirst("FullName")?.Value;
                var student = _context.Students.Include(s => s.Room).FirstOrDefault(s => s.FullName == fullName);
                if (student != null)
                {
                    ViewBag.StudentId = new SelectList(new[] { student }, "Id", "FullName", student.Id);
                    ViewBag.RoomId = new SelectList(new[] { student.Room! }, "Id", "RoomNumber", student.RoomId);
