    ...
    public class ReportsController : Controller
    {
        ...
        public IActionResult MonthlyPayments()
        {
            var report = _context.Payments
                ...
                {
                    MonthYear = g.Key.Year + "-" + g.Key.Month.ToString("D2"),
        ...
        public IActionResult StudentBalance()
        {
            var report = _context.Students
                .Select(s => new StudentBalanceVM
                {
                    ...
                    TotalInvoiced = s.Invoices.Sum(i => (decimal?)(i.Amount + i.PenaltyAmount)) ?? 0,
                    TotalPaid = s.Invoices.SelectMany(i => i.Payments).Sum(p => (decimal?)p.Amount) ?? 0,
                    Balance = (s.Invoices.Sum(i => (decimal?)(i.Amount + i.PenaltyAmount)) ?? 0) - (s.Invoices.SelectMany(i => i.Payments).Sum(p => (decimal?)p.Amount) ?? 0)
                })
